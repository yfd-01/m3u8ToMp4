...
class Crawler:
    ...
    def _merge_set(self, file_path, ts_name_ls):
        ...
        merge_cmd = "ffmpeg -y -f concat -safe 0 -i " + '"' + "ts_ls.txt" + '"' + " -c copy " \
                    + '"' + self.name + ".mp4" + '"'
        p = subprocess.Popen(merge_cmd, shell=True, cwd=file_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print("【INFO】 合并ts文件中...")
        p.communicate()
